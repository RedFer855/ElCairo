...
namespace CapaDeDatos.Repositorios
{
    ...
    {
        // Trae los productos FILTRADOS por la bodega actual
        public async Task<List<Producto>> ObtenerProductosDeBodega(int idBodega)
        {
            try
            {
                ...
                var idsProductos = listaInventario.Select(x => x.IdProducto).ToList();
                ...
                foreach (var prod in listaProductos)
                {
                    ...
                    var datoInventario = listaInventario.FirstOrDefault(inv => inv.IdProducto == prod.IdProducto);
                    if (datoInventario != null)
                    {
                        prod.StockEnBodega = datoInventario.StockActual;
                    }
                }
